...
import Data.HashMap.Strict as HM (HashMap, fromList, (!), member, map, adjust, empty, insertWith, keys)
...
increment :: [[Int]] -> [[Int]]
increment = L.map (L.map (uncurry (+) . (`divMod` 10) . succ))

makeLarge :: Int -> [[Int]] -> [[Int]]
makeLarge n = concatMap (L.map f) . L.take n . iterate increment
 where f = concat . snd . mapAccumL g 0 . replicate n
       g n l = (n+1, L.map (uncurry (+) . (`divMod` 10) . (n+)) l)
...
solve m = sum
 ...
 where (ys,xs) = (length matrixExpanded, length $ head matrixExpanded)
       heuristic = fromIntegral . (grid !)
       grid = HM.fromList $ zip [(x,y) | y <- [0..ys-1], x <- [0..xs-1]] $ concat matrixExpanded
       matrixExpanded = makeLarge 5 m
       goal  = (xs-1,ys-1)
       start = (0,0)
